...
class GeneralizedRCNN_Fnoise(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        #     # features[key] = features[key] + 1e-5*torch.rand(1).cuda()*torch.abs(feat)
        #     # features[key] = features[key] + 1e-10 * torch.rand(1).cuda() * torch.norm(feat, 2) * torch.ones_like(feat)
        #     # features[key] = features[key] + 1e-5 * torch.randn(1).cuda() * torch.norm(feat, p=2, dim=(2, 3), keepdim=True)
        #     features[key] = features[key] + 1e-3 * torch.randn_like(feat).cuda() * torch.norm(feat, p=2, dim=(2, 3),
        #                                                                              keepdim=True)
        #     # features[key] = features[key] + 2e-4 * torch.ones_like(feat) * torch.mean(feat, dim=(2, 3),
        #     #                                                                                   keepdim=True)
